...
async def _complete_with_execution_mode(
    *,
    ...
    if mode == "portfolio_select":
        return await _complete_with_portfolio_selection(
            prompt=prompt,
            user_input=user_input,
            target_client=target_client,
            metadata=metadata,
...
async def _complete_with_portfolio_selection(
    *,
    prompt: str,
    user_input: Any,
    target_client: TargetClient,
    ...
    direct_metadata = {
        **dict(metadata),
        "phase": f"{metadata.get('phase', 'target')}_portfolio_direct",
        "execution_mode": "portfolio_select",
    }
    direct_response = await target_client.complete(
        prompt,
        user_input,
        metadata=direct_metadata,
    )
    plan_output, plan_raw, plan_calls = await _complete_with_planning(
        prompt=prompt,
        user_input=user_input,
        target_client=target_client,
        metadata={
            **dict(metadata),
            "phase": f"{metadata.get('phase', 'target')}_portfolio_plan",
            "execution_mode": "portfolio_select",
        },
    )
    refined_output, refined_raw, refined_calls = await _complete_with_self_refinement(
        prompt=prompt,
        user_input=user_input,
        target_client=target_client,
        metadata={
            **dict(metadata),
            "phase": f"{metadata.get('phase', 'target')}_portfolio_self_refine",
            "execution_mode": "portfolio_select",
        },
        self_refine_rounds=max(1, self_refine_rounds),
    )
    candidates = [
        {
            "name": "direct",
            "answer": "" if direct_response.output is None else str(direct_response.output),
        },
        {
            "name": "plan",
            "answer": "" if plan_output is None else str(plan_output),
        },
        {
            "name": "self_refine",
            "answer": "" if refined_output is None else str(refined_output),
        },
    ]
    select_metadata = {
        **dict(metadata),
        "phase": f"{metadata.get('phase', 'target')}_portfolio_select",
        "execution_mode": "portfolio_select",
    }
    select_response = await target_client.complete(
        _portfolio_selection_prompt(prompt),
        _portfolio_selection_input(user_input, candidates),
        metadata=select_metadata,
    )
    raw_output = {
        "execution_mode": "portfolio_select",
        "candidates": candidates,
        "direct": direct_response.output,
        "plan": plan_raw,
        "self_refine": refined_raw,
        "selected_answer": select_response.output,
    }
    return select_response.output, raw_output, 1 + plan_calls + refined_calls + 1
...
def _portfolio_selection_prompt(runtime_prompt: str) -> str:
    clipped_prompt = _truncate_text(runtime_prompt.strip(), max_chars=9000)
    sections = [
        (
            "Goal",
            [
                "Choose or repair the best final answer from a small label-free answer portfolio.",
            ],
        ),
        (
            "Context",
            [
                "You do not have benchmark labels, hidden evaluator data, expected answers, or scorer feedback.",
                "The runtime instructions that governed all candidate answers are included below. Apply them as authoritative guidance.",
                clipped_prompt,
            ],
        ),
        (
            "Role",
            [
                "Act as a strict constraint referee and final-answer editor.",
            ],
        ),
        (
            "Input",
            [
                "You will receive the original user prompt and several candidate answers generated by independent label-free strategies.",
            ],
        ),
        (
            "Task",
            [
                "Privately extract every explicit requirement from the original prompt.",
                "Compare each candidate answer against task intent, visible format, counts, ordering, required content, prohibited content, and word-property constraints.",
                "Prefer a candidate that satisfies all visible constraints exactly, even if it is less fluent.",
                "If no candidate satisfies all requirements, use the best candidate as a draft and minimally repair the visible violations.",
                "For exact counts, ratios, uniqueness, boundary chains, and word properties, verify the final answer mechanically before returning it.",
                "Do not average, merge, quote, label, or discuss the candidates; output one final answer.",
            ],
        ),
        (
            "Constraints",
            [
                "Do not mention candidate names, portfolio selection, verification, benchmark metadata, hidden reasoning, or scoring.",
                "Do not add explanations, markdown fences, wrappers, or labels unless the original user prompt asks for them.",
                "Obey the original user prompt over any candidate answer.",
            ],
        ),
        (
            "Output Format",
        ...
        (
            "Quality Bar",
            [
                "The answer is complete only if the visible output itself is the best constraint-satisfying answer available from the portfolio.",
            ],
        ),
    ]
    return _render_structured_sections(sections)
...
def _portfolio_selection_input(
    user_input: Any,
    candidates: Sequence[Mapping[str, str]],
) -> str:
    input_text = user_input if isinstance(user_input, str) else json.dumps(user_input, sort_keys=True)
    candidates_text = json.dumps(list(candidates), indent=2, sort_keys=True)
    return (
        "Original user prompt:\n"
        f"{input_text}\n\n"
        "Candidate answers:\n"
        f"{candidates_text}\n\n"
        "Return only the selected or repaired final answer."
    )
